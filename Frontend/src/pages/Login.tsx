...
export default function Login() {
  ...
  const handleLogin = (roleId: any) => {
    login(roleId);
    navigate('/dashboard'); // Send them to the nerve center
  };
    ...
    <div className="min-h-screen bg-cine-void flex flex-col items-center justify-center p-6 relative overflow-hidden selection:bg-cine-gold/30">
      ...
      >
        <button onClick={() => navigate('/')} className="mb-12 flex items-center gap-2 text-cine-dust hover:text-cine-cream transition-colors font-caption tracking-widest text-xs uppercase">
          <ChevronLeft className="w-4 h-4" /> Return to Public Grid
        </button>

        <div className="mb-12 border-l-2 border-cine-gold pl-6">
          <h1 className="font-display text-4xl text-cine-ivory mb-2">Identity Verification</h1>
          <p className="font-mono text-xs text-cine-dust tracking-widest uppercase">Select role to connect to CineCore API.</p>
        </div>
          ...
          {roles.map((role, idx) => {
            ...
            return (
              <motion.button
                ...
                className="group relative flex items-start gap-6 p-6 bg-cine-onyx border border-cine-border hover:border-cine-gold transition-all duration-500 text-left overflow-hidden"
              >
